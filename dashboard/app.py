		...
		if selected_table == "order_items":
			...
			if "latitude" in df_clean.columns and "longitude" in df_clean.columns:
				...
				if "center_name" in df_clean.columns:
					fig, ax = plt.subplots()
					ax.scatter(df_clean["longitude"], df_clean["latitude"], s=50, c=range(len(df_clean)), cmap="viridis", alpha=0.7)
			...
			if all(col in df_clean.columns for col in ["region", "latitude", "longitude"]):
				fig3, ax3 = plt.subplots()
				regions = df_clean["region"].unique()
				colors = plt.cm.tab10(range(len(regions)))
				for i, region in enumerate(regions):
					mask = df_clean["region"] == region
					ax3.scatter(df_clean["longitude"][mask], df_clean["latitude"][mask], s=50, color=colors[i], label=region, alpha=0.7)
			...
			if "latitude" in df_clean.columns and "longitude" in df_clean.columns:
				...
				for i in range(len(coords)):
					for j in range(i + 1, len(coords)):
						distances.append(haversine(coords[i][0], coords[i][1], coords[j][0], coords[j][1]))
				if distances:
					fig4, ax4 = plt.subplots()
			...
			st.info("Custom visualizations for this table are not yet defined.")

		st.subheader("Daily Orders Trend")
		if 'order_date' in kpis.columns and 'orders' in kpis.columns:
			fig_orders, ax_orders = plt.subplots()
			ax_orders.plot(kpis['order_date'], kpis['orders'], label='Orders', color='orange')
			ax_orders.set_xlabel('Date')
			ax_orders.set_ylabel('Orders')
			ax_orders.set_title('Daily Orders')
			plt.setp(ax_orders.get_xticklabels(), rotation=45, ha='right')
			fig_orders.tight_layout()
			st.pyplot(fig_orders)
		else:
			st.warning("Required columns 'order_date' and/or 'orders' not found in KPIs data. Unable to plot Daily Orders.")

		st.subheader("Average Order Value Trend")
		if 'order_date' in kpis.columns and 'avg_order_value' in kpis.columns:
			fig_aov, ax_aov = plt.subplots()
			ax_aov.plot(kpis['order_date'], kpis['avg_order_value'], label='Avg Order Value', color='green')
			ax_aov.set_xlabel('Date')
			ax_aov.set_ylabel('Avg Order Value')
			ax_aov.set_title('Average Order Value by Day')
			plt.setp(ax_aov.get_xticklabels(), rotation=45, ha='right')
